...
import eu.righettod.pocauthztesting.enumeration.PayloadLocation;
...
public class AuthorizationMatrixIT {
     ...
     */
    @Test
    public void testAccessUsingAnonymousUserPointOfView() {
     ...
     */
    private List<String> executeTestWithPointOfView(SecurityRole pointOfView, String accessToken) {
        ...
        services.forEach(service -> {
            ...
            PayloadLocation payloadLocation = PayloadLocation.URL;
            ...
            if (serviceTesting.isPresent()) {
                ...
                if (serviceTesting.get().getPayload().getContentType() != null) {
                    payloadContentType = serviceTesting.get().getPayload().getContentType();
                }
                if (serviceTesting.get().getPayload().getLocation() != null) {
                    payloadLocation = PayloadLocation.valueOf(serviceTesting.get().getPayload().getLocation());
                }
            }
            //Call the service and verify if the response is consistent
            try {
                //Call the service
                int serviceResponseCode = callService(service.getUri(), payload, payloadContentType, payloadLocation, service.getHttpMethod(), accessToken);
     ...
     * @param payloadLocation    Location where the payload must be added
     ...
     * @throws Exception If any error occurs
     */
    private int callService(String uri, String payload, String payloadContentType, PayloadLocation payloadLocation, String httpMethod, String accessToken) throws Exception {
        int rc;

        //Build the request - Use Apache HTTP Client in order to be more flexible in the combination
        HttpRequestBase request;
        String url = BASE_URL;
        if (payload != null && payloadLocation.equals(PayloadLocation.URL)) {
            url += "/" + payload.trim();
        } else {
            url += uri;
        }
        switch (httpMethod) {
            ...
            case "PUT":
                ...
                if (payload != null && payloadLocation.equals(PayloadLocation.BODY)) {
