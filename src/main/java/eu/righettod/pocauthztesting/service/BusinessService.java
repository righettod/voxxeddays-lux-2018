...
public class BusinessService {
    ...
    private static boolean CAUSE_EXPLICIT_AUTHZ_IMPL_ERRORS = Boolean.parseBoolean(System.getProperty("CAUSE_EXPLICIT_AUTHZ_IMPL_ERRORS", "false"));
    ...
    public MessageActionResult create(@RequestHeader("Authorization") String accessToken, @RequestBody Message msg, HttpServletResponse response) {
        ...
        try {
            ...
            DecodedJWT token = (accessToken != null && !accessToken.trim().isEmpty()) ? this.validateAccessToken(accessToken) : null;
            if (token == null || (!SecurityRole.ADMIN.name().equalsIgnoreCase(token.getClaim("role").asString()) && !SecurityRole.BASIC.name().equalsIgnoreCase(token.getClaim("role").asString()))) {
                ...
                newMsg.setIdentifier(Integer.toString(MESSAGE_COUNTER));
     ...
     * Delete a message.
     * <p>
     * <code>curl -X DELETE -H "Authorization: eyJ0e..." http://localhost:8080/[NUMBER]</code>
     *
     * @param accessToken Access token taken from the Authorization HTTP header
    ...
    public MessageActionResult delete(@RequestHeader("Authorization") String accessToken, @PathVariable String messageId, HttpServletResponse response) {
        ...
        try {
            ...
            DecodedJWT token = (accessToken != null && !accessToken.trim().isEmpty()) ? this.validateAccessToken(accessToken) : null;
            if ((token == null || (!SecurityRole.ADMIN.name().equalsIgnoreCase(token.getClaim("role").asString()))) && !CAUSE_EXPLICIT_AUTHZ_IMPL_ERRORS) {
                resultStatusInfo.setMessageIdentifier("");
                resultStatusInfo.setSucceed(false);
                resultStatusInfo.setReason("Access denied");
                response.setStatus(HttpServletResponse.SC_FORBIDDEN);
     ...
     * <p>
     * <code>curl -X GET http://localhost:8080/[NUMBER]</code>
